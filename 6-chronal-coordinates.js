const chronalCoordinatesPartOne = (input) => {
  ...
  const grid = gridHeight.map(y => {
    return gridWidth.map(x => {
      return coords.reduce((a, coord) => {
...
const chronalCoordinatesPartTwo = (input, threshold) => {
  const coords = input.map(coord => {
    const [x, y] = coord.match(/\d+/g).map(n => Number(n));
    return { x, y };
  });
  ...
  const grid = gridHeight.map(y => {
    return gridWidth.map(x => {
      const distanceToCoords = coords.reduce((a, coord) => {
        return a + (Math.abs((coord.x - x)) + Math.abs((coord.y - y)));
      }, 0);
      return distanceToCoords < threshold ? 1 : 0;
    });
  });

  return grid.reduce((a, row) => row.reduce((b, loc) => (b + loc), a), 0);
};
